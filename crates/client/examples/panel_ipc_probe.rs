...
fn main()->anyhow::Result<()> {
    ...
    let running=Arc::new(AtomicBool::new(true));let receiver=PanelIpc::engine(running.clone())?;
    assert!(PanelIpc::insert(Request{hit:hit.clone(),target:"probe".into(),created_ms:0}).is_err());assert!(receiver.try_recv().is_err());
    let client=std::thread::spawn(move||PanelIpc::insert(Request{hit,target:"probe".into(),created_ms:SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis()}));
    let request=receiver.recv_timeout(Duration::from_secs(2))?;assert_eq!(request.text,"Safe");assert_eq!(request.target,"probe");request.reply.send(Ok(()))?;client.join().unwrap()?;
