...
test('scheduler registers protected Trash, trial, Helpmonks, signup notification and white-label jobs without overlap', async () => {
	FakeCron.instances = [];
	let release;
	...
	await FakeCron.instances[5].callback();
	assert.equal(purges, 1);
	assert.deepEqual(logs, ['Trash cleanup complete: purged 2 libraries and 3 snippets', 'Trial lifecycle complete: downgraded 2 accounts to Free', 'Helpmonks trial sequence complete: enrolled 1, retrying 0, failed 0', 'Signup notifications complete: sent 1, retrying 0, failed 0', 'White-label reconciliation complete: checked 1, updated 1, failed 0']);
});
