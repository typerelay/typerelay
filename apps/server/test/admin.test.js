...
test('account creation, shared users, counts, search, CSV and revisions', async () => {
	...
	const staleClient = await Fixture.json('/admin/api/accounts', 'POST', { name: 'Stale browser account', owner_name: 'Stale Browser', owner_email: randomUUID() + '@example.test', password: 'F'.repeat(32), plan: 'free' }, 201); assert.equal(staleClient.account.effective_plan, 'free'); assert.equal(Fixture.emails.length, emailCount + 1);
...
test('purge drains in-flight operations, retries external failure, removes every owned record and keeps shared users', async () => {
	...
	for (const name of AdminAccounts.owned.filter(name => name !== 'Member')) await M[name].create({ account: account._id, user: orphan._id, id: name, name, snippet: name, operation: randomUUID(), hash: randomUUID(), ...(name === 'UsageEvent' ? { event_id: randomUUID(), library: new M.mongoose.Types.ObjectId(), snippet: 'deleted', action: 'copy', occurred_at: new Date(), characters: 10 } : {}), ...(name === 'SignupNotification' ? { email: orphan.email, message_id: '<purge@typerelay.test>' } : {}) });
