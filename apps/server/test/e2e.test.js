...
test('web AJAX updates only affected snippets; preserves panel, filter and multiline', async () => {
	...
	const exportText = 'abbreviation,snippet,label\nUPPER,"\tImported\ntext  ",Imported title\nBAD!,Skip,Skip\n' + Array.from({ length: 16 }, (_, index) => 'extra' + index + ',Body ' + index + ',Extra ' + index + '\n').join('');
	...
	assert.equal(dom.window.document.querySelectorAll('[data-import-entry]').length, 18);
	assert.equal(dom.window.document.querySelectorAll('[data-import-entry]:not([hidden])').length, 15);
	assert.equal(dom.window.document.querySelector('[data-import-page-label]').textContent, 'Page 1 of 2');
	assert.equal(dom.window.document.querySelector('[data-import-page="previous"]').disabled, true);
	await client.onClick({ target: dom.window.document.querySelector('[data-import-page="next"]') });
	assert.equal(dom.window.document.querySelectorAll('[data-import-entry]:not([hidden])').length, 3);
	assert.equal(dom.window.document.querySelector('[data-import-page-label]').textContent, 'Page 2 of 2');
	importFile.dispatchEvent(new dom.window.Event('change', { bubbles: true }));
	assert.equal(dom.window.document.querySelectorAll('[data-import-entry]').length, 0);
	assert.equal(dom.window.document.querySelector('#record-form button[type="submit"]').disabled, true);
	await client.onClick({ target: dom.window.document.querySelector('#preview-import') });
	assert.equal(dom.window.document.querySelector('[data-import-page-label]').textContent, 'Page 1 of 2');
	const topImport = dom.window.document.querySelector('#import-submit-top');
	const bottomImport = dom.window.document.querySelector('#record-form .d-flex.justify-content-between button[type="submit"]');
	assert.equal(topImport.disabled, false);
	...
	await client.onClick({ target: dom.window.document.querySelector('[data-import-page="next"]') });
	const pageTwoCorrection = dom.window.document.querySelector('[data-import-trigger="0:15"]');
	pageTwoCorrection.value = 'page-two';
	const pageTwoUnselected = dom.window.document.querySelector('[data-import-key="0:17"]');
	pageTwoUnselected.checked = false; pageTwoUnselected.dispatchEvent(new dom.window.Event('change', { bubbles: true }));
	await client.onClick({ target: dom.window.document.querySelector('[data-import-page="previous"]') });
	assert.equal(correction.value, 'beta-ui');
	assert.equal(unselected.checked, false);
	await client.onClick({ target: dom.window.document.querySelector('[data-import-page="next"]') });
	assert.equal(pageTwoCorrection.value, 'page-two');
	assert.equal(pageTwoUnselected.checked, false);
	for (const input of dom.window.document.querySelectorAll('[data-import-key]:checked')) { input.checked = false; input.dispatchEvent(new dom.window.Event('change', { bubbles: true })); }
	assert.equal(topImport.disabled, true);
	assert.equal(bottomImport.disabled, true);
	for (const input of dom.window.document.querySelectorAll('[data-import-key]')) if (!['0:1', '0:17'].includes(input.dataset.importKey)) { input.checked = true; input.dispatchEvent(new dom.window.Event('change', { bubbles: true })); }
	assert.equal(topImport.disabled, false);
	assert.equal(bottomImport.disabled, false);
	const oldCount = client.libraries.size;
	client.open = async () => { throw new Error('Import must not reload editor'); };
	const importRequest = client.request.bind(client);
	const submitted = [];
	client.request = (path, ...args) => { if (path === 'import/textexpander') submitted.push(args[1].selected); return importRequest(path, ...args); };
	const form = dom.window.document.querySelector('#record-form');
	await client.onSubmit({ target: form, submitter: topImport, preventDefault() {} });
	await client.onSubmit({ target: form, submitter: bottomImport, preventDefault() {} });
	assert.deepEqual(submitted[0], submitted[1]);
	assert.equal(submitted[0].length, 16);
	assert.ok(submitted[0].some(entry => entry.key === '0:15' && entry.trigger === 'page-two'));
