...
const variableDefinition = { type: 'object', properties: { label: string, default: string, required: { type: 'boolean' }, multiline: { type: 'boolean' }, format: { enum: ['', 'YYYY-MM-DD', 'DD/MM/YYYY', 'MM/DD/YYYY', 'HH:mm', 'HH:mm:ss', 'YYYY-MM-DD HH:mm', 'ISO'] }, timezone: { enum: ['local', 'utc'] } }, additionalProperties: false };
const content = { type: 'object', required: ['version', 'type', 'text'], properties: { version: { const: 1 }, type: { enum: ['plain_text', 'code', 'template'] }, text: string, language: string, variables: { type: 'object', additionalProperties: variableDefinition } } };
const value = { example: { title: 'Greeting', trigger: 'hello', content: { version: 1, type: 'plain_text', text: 'Hello!' } }, type: 'object', required: ['content'], properties: { title: string, trigger: { type: ['string', 'null'] }, content } };
