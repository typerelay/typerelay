...
export class TemplateFill {
	...
	async open(content, revalidate = null, onCopied = null) {
		...
		this.content = this.rich ? source : result.template; this.revalidate = revalidate; this.onCopied = onCopied;
		if (!result.fields.length) { if (revalidate) await revalidate(); const filled = this.rich ? await this.client.copyRich(source) : await TemplateRuntime.render(source); if (!this.rich) await navigator.clipboard.writeText(filled.text); onCopied?.(filled.text, filled.characters); this.client.toast(filled.enter_actions ? 'Copied content. Enter key actions were omitted.' : 'Copied'); return; }
