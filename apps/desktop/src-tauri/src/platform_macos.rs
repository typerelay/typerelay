...
unsafe extern "C" { fn AXIsProcessTrusted() -> bool; fn AXUIElementCreateApplication(pid:i32)->Ref; fn AXUIElementCopyAttributeValue(element:Ref,attribute:Ref,value:*mut Ref)->i32; fn AXUIElementPerformAction(element:Ref,action:Ref)->i32; fn AXValueGetValue(value:Ref,kind:i32,result:*mut c_void)->bool; fn CGEventSourceKeyState(source:i32,key:u16)->bool; fn CGEventKeyboardGetUnicodeString(event:core_graphics::sys::CGEventRef,max:usize,actual:*mut usize,buffer:*mut u16); fn CGPreflightListenEventAccess()->bool; fn CGRequestListenEventAccess()->bool; }
...
pub fn input_monitoring(prompt:bool)->bool { let trusted=unsafe{CGPreflightListenEventAccess()};if !trusted&&prompt{unsafe{CGRequestListenEventAccess()}}else{trusted} }
...
pub fn open_input_monitoring_settings()->Result<()> { let _=input_monitoring(true);let status=Command::new("/usr/bin/open").arg("x-apple.systempreferences:com.apple.preference.security?Privacy_ListenEvent").status()?;ensure!(status.success(),"Could not open Input Monitoring settings");Ok(()) }
