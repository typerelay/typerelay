...
impl Target {
    ...
    fn message(&self,message:u32,value:usize)->Result<()> {let window=HWND(self.handle as *mut _);let thread=unsafe{GetWindowThreadProcessId(window,None)};let mut info=GUITHREADINFO{cbSize:std::mem::size_of::<GUITHREADINFO>() as u32,..Default::default()};unsafe{GetGUIThreadInfo(thread,&mut info)?;}ensure!(!info.hwndFocus.0.is_null(),"Notepad edit control is unavailable");let sent=unsafe{SendMessageTimeoutW(info.hwndFocus,message,WPARAM(value),LPARAM(1),SMTO_ABORTIFHUNG,500,None)};ensure!(sent.0!=0,"Notepad did not accept insertion");Ok(())}
    fn message_insert(&self,erase:usize,paste:bool)->Result<()> {for _ in 0..erase{self.message(WM_CHAR,VK_BACK.0 as usize)?;}self.message(if paste{WM_PASTE}else{WM_CHAR},if paste{0}else{'\r' as usize})}
}
