...
impl ksni::Tray for Tray {
	...
	fn menu(&self)->Vec<ksni::MenuItem<Self>>{use ksni::menu::StandardItem;vec![StandardItem{label:"Sync now".into(),activate:Box::new(|tray:&mut Self|{let _=crate::sync_now(tray.0.clone());}),..Default::default()}.into(),StandardItem{label:"Check for updates…".into(),activate:Box::new(|tray:&mut Self|crate::update::check(tray.0.clone(),true)),..Default::default()}.into(),StandardItem{label:"Settings".into(),activate:Box::new(|tray:&mut Self|{let app=tray.0.clone();let _=tray.0.run_on_main_thread(move||Runtime::open(&app,true));}),..Default::default()}.into(),StandardItem{label:"Quit TypeRelay".into(),activate:Box::new(|tray:&mut Self|Runtime::quit(&tray.0)),..Default::default()}.into()]}
}
