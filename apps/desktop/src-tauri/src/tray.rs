...
pub fn install(app:&tauri::AppHandle)->Result<()> {
    ...
    #[cfg(not(target_os="linux"))]
    {
		...
		let menu=MenuBuilder::new(app).item(&sync).item(&settings).item(&updates).item(&quit).build()?;
