...
pub fn install(app:&tauri::AppHandle)->Result<()> {
    ...
    #[cfg(not(target_os="linux"))]
    {
        ...
        TrayIconBuilder::with_id("typerelay").icon(tauri::image::Image::from_bytes(icon)?).icon_as_template(cfg!(target_os="macos")).tooltip("TypeRelay").menu(&menu).show_menu_on_left_click(false).on_menu_event(|app,event|match event.id.as_ref(){"sync"=>{let _=crate::sync_now(app.clone());},"settings"=>Runtime::open(app,true),"quit"=>Runtime::quit(app),_=>()}).on_tray_icon_event(|tray,event|{if matches!(event,TrayIconEvent::Click{button:MouseButton::Left,button_state:MouseButtonState::Up,..}){Runtime::open(tray.app_handle(),false);}}).build(app)?;
    }
    Ok(())
}
