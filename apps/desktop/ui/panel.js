class Panel {
	...
	configure(value){document.querySelector('#shortcut').value=value.config.shortcut;document.querySelector('#autostart').checked=value.config.launch_at_login;if(value.server)document.querySelector('#server').value=value.server;for(const [name,allowed]of[['accessibility',!!value.accessibility],['input-monitoring',!!value.input_monitoring]]){const card=document.querySelector('#'+name+'-card');const state=document.querySelector('#'+name+'-state');state.textContent=allowed?'Approved':'Required';state.dataset.allowed=String(allowed);card.hidden=false;card.querySelector('button').hidden=allowed;}}
