class Panel {
	...
	async refreshPermissions(){try{const value=await this.invoke('initialize');this.configure(value);this.status.textContent=value.status||'';}catch(error){this.status.textContent=String(error);}}
