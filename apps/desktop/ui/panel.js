class Panel {
	constructor() {
		...
		document.querySelector('#connect-form').onsubmit=event=>{event.preventDefault();this.action(async()=>{await this.notify('Complete sign-in in your browser…');await this.invoke('connect',{url:document.querySelector('#server').value});await this.notify('Connected');});};
		...
		window.__TAURI__.event.listen('sync-notice',async event=>{const button=document.querySelector('#sync');button.disabled=event.payload.running;button.textContent=event.payload.running?'Syncing…':'Sync now';if(!event.payload.running){try{await this.refreshLibraries();}catch(error){this.notify(error,true);}}});
	...
	settingsPage(name){this.settingsTab=name;for(const button of document.querySelectorAll('[data-settings-tab]'))button.setAttribute('aria-selected',String(button.dataset.settingsTab===name));document.querySelector('#settings-general').hidden=name!=='general';document.querySelector('#settings-sync').hidden=name!=='sync';}
