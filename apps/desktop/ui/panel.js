class Panel {
	constructor() {
		...
		window.addEventListener('focus',()=>{if(document.body.dataset.view==='settings')this.refreshAccessibility();});
		this.invoke('initialize').then(value=>{this.configure(value);this.open(value);}).catch(error=>this.status.textContent=String(error));
	}
	configure(value){document.querySelector('#shortcut').value=value.config.shortcut;document.querySelector('#autostart').checked=value.config.launch_at_login;if(value.server)document.querySelector('#server').value=value.server;const accessibility=document.querySelector('#accessibility-state');accessibility.textContent='Required';accessibility.dataset.allowed=String(!!value.accessibility);document.querySelector('#accessibility-card').hidden=!!value.accessibility;}
	async refreshAccessibility(){try{const value=await this.invoke('initialize');this.configure(value);if(value.accessibility&&this.status.textContent.includes('Accessibility'))this.status.textContent='';}catch(error){this.status.textContent=String(error);}}
