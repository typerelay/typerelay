class Panel {
	constructor() {
		...
		document.querySelector('#open-accessibility').onclick=()=>this.action(async()=>{await this.invoke('open_accessibility_settings');this.status.textContent='Enable TypeRelay in Accessibility, then return here.';});
		document.querySelector('#open-notifications').onclick=()=>this.action(async()=>{await this.invoke('open_notification_settings');});
		document.querySelector('#open-input-monitoring').onclick=()=>this.action(async()=>{await this.invoke('open_input_monitoring_settings');this.status.textContent='Enable TypeRelay in Input Monitoring, then return here.';});
	...
	statusMessage(value){const status=String(value||'');return /^Choose (?:another|an) (?:app|application)\b/i.test(status)?'':status;}
	async refreshPermissions(){try{const value=await this.invoke('initialize');this.configure(value);this.status.textContent=this.statusMessage(value.status);}catch(error){this.status.textContent=String(error);}}
