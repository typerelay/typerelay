...
export class LocalMacBuild {
	...
	static async signingIdentity(environment = process.env) { const output = await LocalMacBuild.output('security', ['find-identity', '-v', '-p', 'codesigning']); const identities = [...output.matchAll(/"(Apple Development:[^"]+)"/g)].map(match => match[1]); const requested = environment.APPLE_DEVELOPMENT_SIGNING_IDENTITY; if (requested && identities.includes(requested)) return requested; if (identities.length === 1) return identities[0]; throw new Error('Set APPLE_DEVELOPMENT_SIGNING_IDENTITY to one installed Apple Development identity'); }
	static async main(platform = process.platform) {
		...
		await LocalMacBuild.run('codesign', ['--force', '--sign', identity, '--identifier', 'com.typerelay.tui', '--timestamp=none', tui]);
		await LocalMacBuild.run('codesign', ['--force', '--sign', identity, '--identifier', config.identifier, '--timestamp=none', app]);
		...
		console.log('Built Apple Development-signed TypeRelay app and DMG for local testing: ' + dmg);
	}
}
