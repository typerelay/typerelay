...
export class Auth {
 static callback = 'com.typerelay.mobile://oauth/callback';
 ...
 static isCallback(value: string) { try { const url = new URL(value); return url.protocol === 'com.typerelay.mobile:' && url.hostname === 'oauth' && url.pathname === '/callback' && !url.port && !url.username && !url.password && !url.hash; } catch { return false; } }
 static normalize(value: string) { const url = new URL(value); if (url.protocol !== 'https:' || url.username || url.password || url.pathname !== '/' || url.search || url.hash) throw new Error('Enter an HTTPS server origin without a path.'); return url.origin; }
 ...
 static async begin(value: string) {
  const server = Auth.normalize(value); const verifier = Auth.random(); const state = Auth.random();
  ...
  await Browser.open({ url: `${server}/oauth/authorize?${params}` });
 }
 static async complete(value: string) {
  ...
  try { await Native.call('bind', { server: pending.server, account: tokens.account }); }
  catch (error) { await CapacitorHttp.delete({ url: `${pending.server}/api/v2/connection`, headers: { Authorization: `Bearer ${tokens.access_token}`, 'X-TypeRelay-Sync-Protocol': '6' }, disableRedirects: true }).catch(() => undefined); throw error; }
 ...
 static async tokenRequest(server: string, body: Record<string,string>) { const result = await CapacitorHttp.post({ url: `${server}/oauth/token`, headers: { 'Content-Type': 'application/json' }, data: body, disableRedirects: true }); if (result.status !== 200) { if (result.status === 401 && body.grant_type === 'refresh_token') await Native.call('suspend'); throw new Error(result.data?.error || 'Sign-in failed'); } return { ...result.data, obtained_at: Date.now() } as Tokens; }
 ...
 static async request(path: string, method: string, data?: unknown) {
  const send = async (force: boolean) => CapacitorHttp.request({ url: `${Auth.server}/api/v2/${path}`, method, headers: { Authorization: `Bearer ${await Auth.access(force)}`, 'X-TypeRelay-Sync-Protocol': '6', 'Content-Type': 'application/json' }, data, disableRedirects: true });
