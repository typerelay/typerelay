...
- macOS: global hotkey + AppKit/Accessibility window identity and activation, CoreGraphics key-state checks, Enigo paste, template T status icon and a decorated overlay window with native traffic lights and popover material. Startup checks Accessibility and uses the macOS permission prompt when access is missing; Settings can reopen the Accessibility pane. Local test builds use an Apple Development identity so the permission survives rebuilds and restarts without Developer ID distribution signing or notarization. A clipboard with multiple items is left untouched and automatic insertion is refused; Copy remains explicit. The app bundles **TypeRelay TUI** and opens it in Terminal from the menu or Settings.
