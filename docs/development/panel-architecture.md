...
- macOS: global hotkey + a passive CoreGraphics session event tap, Accessibility window identity and activation, marked CoreGraphics insertion events, template T status icon and a decorated overlay window with native traffic lights and popover material. The event marker prevents inserted text from feeding the abbreviation engine again. Settings uses compact General/Sync segmented views and unified macOS preference rows instead of a long form. Startup checks Accessibility and Input Monitoring separately, opens the matching System Settings panes and starts continuous expansion only after both permissions are active. Local credentialed test builds use the installed Developer ID Application identity so approvals survive rebuilds and restarts. A clipboard with multiple items is left untouched and automatic insertion is refused; Copy remains explicit. The app bundles **TypeRelay TUI** and opens it in Terminal from the menu or Settings.
- Windows: global hotkey plus a low-level keyboard hook, HWND + retained process handle, foreground verification, confirming-key release checks and native input. The foreground window's keyboard layout translates virtual/scancodes through `ToUnicodeEx`, including Shift, Caps Lock and AltGr; shortcut modifiers cancel matching. TypeRelay marks its own `SendInput` events and ignores only that marker, allowing other software and remote-control input without matching its own insertion. Snapshots/settings reload without restarting, and expansion is paused in TypeRelay TUI. Clipboard preservation snapshots movable formats by native numeric ID. RDP clipboard providers can refuse snapshots; that path still inserts through the clipboard and leaves the inserted text there. Elevated/protected applications can reject input; TypeRelay does not elevate itself to bypass that.
...
Windows Notepad and PowerShell pass direct expansion plus keyboard and one-click panel insertion in the test VM. macOS TextEdit passes direct expansion and one-click panel insertion. The broader platform checklists remain pending.
...
8. Launch **TypeRelay TUI** from the macOS menu/Settings or Windows Start, edit a snippet, sync, and confirm the panel and web app show the same revision. Windows uninstall must remove the TUI shortcut while preserving the local database.
