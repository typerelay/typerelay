...
Automated checks cover portable search ordering, stale selection, permissions/lifecycle through existing database tests, shortcut validation, private IPC expiry and nonblocking full-socket behavior, installer ownership/uninstall, UI stale-search/Enter handling and existing engine/TUI tests. Both Cargo workspaces pass warnings-denied Clippy locally. GitHub builds produce macOS Apple Silicon app/dmg, a Windows x64 NSIS installer containing the panel, CLI and TUI, and Linux packages/Omarchy bundle.
