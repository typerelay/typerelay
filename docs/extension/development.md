...
A fresh install opens the options page for sign-in. The toolbar opens settings when signed out, and its Settings button opens them after sign-in. Settings has Sync now and the trigger prefix. Before signing in, enter a server hostname beside `https://` and submit Sign in; approve Chrome’s server permission if prompted. The selected server is saved for login, sync, and app links. The small reset control restores `app.typerelay.com` in the field; submit Sign in to use it. Sign out before switching servers. The popup starts with an empty focused search field, shows up to 20 matches only after typing, and supports Up/Down to select a snippet. Enter or clicking a result copies it and closes the popup; press Ctrl/Cmd+V in the destination to paste. Rich snippets copy HTML (including embedded image data) and a plain-text representation. Variable inputs open inside the popup; Copy submits and Cancel returns to search. Snippets containing executable Enter-key actions cannot be copied. Copying works without an editable page or native messaging bridge.
...
Google Docs uses the popup copy-and-manual-paste workflow. Automatic abbreviation expansion in Docs remains unsupported. Before Chrome Web Store submission, manually verify Enter and click copying, popup closure, paste at the original cursor/selection, multiline text, Unicode, variable submission/cancellation, formatting, links, and images in Docs and another rich editor. Also verify plain-text destinations, copying without the desktop bridge or a focused editor, and unchanged abbreviation expansion on supported sites. Formatting and image acceptance depend on the destination. Keep the listing staged until these checks pass; packaging alone does not clear this gate.

The extension requests clipboardWrite to copy snippets; it does not request clipboardRead. The Web Store listing must disclose clipboard modification, broad site access, local snippet and image caching, sign-in tokens, and native messaging. Package only the `dist` directory. Keep all executable code in the package.
