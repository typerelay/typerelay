...
Backspace, Delete and unmodified Left/Right arrows can edit within a typed abbreviation before Space. Move the caret back to the end before pressing Space to expand. Modifier shortcuts, pointer clicks, focus changes, moves outside the abbreviation and unsupported navigation cancel the pending match instead of inserting into an uncertain target.
